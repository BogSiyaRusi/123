...
jobs:
  # ── Static Analysis: detect suspicious JS patterns ──
  static-analysis:
    ...
    steps:
      ...
      - name: Detect network requests (fetch/XMLHttpRequest/WebSocket)
        run: |
          ...
          if grep -rPn "$PATTERNS" --include="*.js" --include="*.mjs" . 2>/dev/null; then
            ...
            # Don't fail — some VK API calls are legitimate. Flag for review.
          else
            echo "✅ No network request patterns found."
          fi
      ...
      - name: Detect dynamic code execution
        run: |
          ...
          FAIL=0
          # eval() — always dangerous
          if grep -rPn '\beval\s*\(' --include="*.js" --include="*.mjs" . 2>/dev/null; then
            echo "::error::eval() detected — this is prohibited."
            FAIL=1
          fi
          # new Function() — code generation
          if grep -rPn 'new\s+Function\s*\(' --include="*.js" --include="*.mjs" . 2>/dev/null; then
            echo "::error::new Function() detected — dynamic code generation is prohibited."
            FAIL=1
          fi
          # document.write
          if grep -rPn 'document\.write\s*\(' --include="*.js" --include="*.mjs" . 2>/dev/null; then
            echo "::error::document.write() detected — this is prohibited."
            FAIL=1
          fi
          # setTimeout/setInterval with string argument
          if grep -rPn '(setTimeout|setInterval)\s*\(\s*["\x27`]' --include="*.js" --include="*.mjs" . 2>/dev/null; then
            echo "::error::setTimeout/setInterval with string argument detected — use function references instead."
            FAIL=1
          fi
          if [ $FAIL -eq 0 ]; then
            echo "✅ No dynamic code execution patterns found."
          else
            exit 1
          fi
      ...
      - name: Detect external URL references
        run: |
          ...
          # Allow vk.com, vk.ru, vkvideo.ru, userapi.com (VK CDN) — block everything else
          ...
          if [ -n "$EXTERNAL" ]; then
            ...
            echo "✅ No suspicious external URLs found."
          fi
      ...
      - name: Detect obfuscation patterns
        run: |
          ...
          FAIL=0
          # Very long single lines (>5000 chars) that aren't CSS — possible obfuscated payload
          ...
          # atob() — base64 decode; used legitimately in atou() for theme sharing,
          ...
          if [ "$ATOB_COUNT" -gt 1 ]; then
            echo "::error::Multiple atob() calls detected ($ATOB_COUNT) — only the existing atou() wrapper is allowed."
            FAIL=1
          elif [ "$ATOB_COUNT" -eq 1 ]; then
            echo "ℹ️ Single atob() found (expected: atou helper for theme sharing)."
          fi
          # String.fromCharCode with many args — character-by-character string building
          if grep -rPn 'String\.fromCharCode\s*\([^)]{20,}\)' --include="*.js" --include="*.mjs" . 2>/dev/null; then
            echo "::error::String.fromCharCode with many arguments detected — possible obfuscation."
            FAIL=1
          fi
          if [ $FAIL -eq 0 ]; then
      ...
      - name: Detect cookie/storage manipulation
        run: |
          ...
          if grep -rPn '\b(localStorage|sessionStorage)\.(setItem|getItem|removeItem|clear)\b' --include="*.js" --include="*.mjs" . 2>/dev/null; then
            echo "ℹ️ Storage API usage detected (informational, may be legitimate)."
          fi
          echo "✅ Cookie/storage scan complete."
      ...
      - name: Detect VK API abuse patterns
        run: |
          ...
          if grep -rPn "$ABUSE_PATTERNS" --include="*.js" --include="*.mjs" . 2>/dev/null; then
            echo "::error::Suspicious VK API calls detected — these methods were used in the original malware."
            FAIL=1
          fi
          if [ $FAIL -eq 0 ]; then
            echo "✅ No VK API abuse patterns found."
          else
            exit 1
          fi
      ...
      - name: Check for dangerous permissions
        run: |
          ...
          if grep -q '"update_url"' manifest.json; then
            echo "::error::update_url found — auto-update from external server is prohibited."
            FAIL=1
          fi
          # Check for key (extension identity pinning)
          if grep -q '"key"' manifest.json; then
            echo "::error::'key' field found — extension identity should not be pinned."
            FAIL=1
          fi
          if [ $FAIL -eq 0 ]; then
            echo "✅ No dangerous permissions or fields found."
          else
            exit 1
          fi
      ...
      - name: Check content_security_policy
        run: |
          if grep -qP "unsafe-eval|unsafe-inline" manifest.json; then
            echo "::error::CSP contains unsafe-eval or unsafe-inline — this is prohibited."
            exit 1
          fi
          echo "✅ CSP check passed."

  # ── JSON & locale validation ──
  json-lint:
    name: JSON Lint
    runs-on: ubuntu-latest
    steps:
      ...
      - name: Detect dangerous CSS patterns
        run: |
          ...
          if grep -Pin '\-moz\-binding' css mcss 2>/dev/null; then
            echo "::error::-moz-binding detected — this is a known XSS vector."
            FAIL=1
          fi
          # expression() — IE CSS expression (code execution)
          if grep -Pin 'expression\s*\(' css mcss 2>/dev/null; then
            echo "::error::CSS expression() detected — this allows code execution."
            FAIL=1
          fi
          # @import with external URL
          if grep -Pin '@import\s+(url\s*\()?\s*["\x27]?https?://' css mcss 2>/dev/null; then
            echo "::error::External @import detected — loading external stylesheets is prohibited."
            FAIL=1
          fi
          if [ $FAIL -eq 0 ]; then
            echo "✅ No dangerous CSS patterns found."
          fi
          exit $FAIL

  # ── Diff analysis for PRs ──
  diff-review:
        ...
        run: |
          for f in 0.js 1.js 2.js manifest.json; do
            if git diff --name-only origin/${{ github.base_ref }}...HEAD | grep -q "^${f}$"; then
              ...
              echo "📊 $f: +$ADDITIONS / -$DELETIONS lines"
              if [ "$ADDITIONS" -gt 50 ]; then
                echo "::warning file=$f::Large addition (+$ADDITIONS lines) to critical file $f — requires careful security review."
              fi
            fi
          done
            ...
            for f in $NEW_FILES; do
              case "$f" in
                *.exe|*.dll|*.so|*.dylib|*.bat|*.cmd|*.ps1|*.sh)
                  echo "::error::Executable file added: $f — this is prohibited."
                  exit 1
                  ;;
                *.wasm)
                  echo "::error::WebAssembly file added: $f — binary code is prohibited."
                  exit 1
                  ;;
              esac
            done
    ...
    steps:
        ...
        run: |
          ...
          for locale in en ru uk; do
            if [ ! -f "_locales/$locale/messages.json" ]; then
              ...
              echo "✅ _locales/$locale/messages.json exists"
            fi
          done
          exit $FAIL
      ...
      - name: Check for unexpected file types
        run: |
          ...
          echo "✅ No suspicious file types found."# 123
